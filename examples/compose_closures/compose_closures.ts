...
export async function compose_B(env: Env, f: (env: Env) => Promise<number>, g: (env: Env, arg0: number) => Promise<void>): Promise<(env: Env) => Promise<void>> {
  return async (env: Env) => {
    await g(env, env.copy(await f(env)));
  };
}
...
export async function Start_A(env: Env, input: number) {
  let f: (env: Env, arg0: number) => Promise<void> = env.copy(async (env: Env, value: number) => {
    await incAndSend_X(env.subst("A", "X", "B", "Y"), value);
  });
  let c: (env: Env, arg0: number) => Promise<void> = env.copy(await compose_A(env.subst("A", "A", "B", "B", "C", "C"), env.copy(f)));
  await c(env, env.copy(input));
}
export async function Start_B(env: Env) {
  let f: (env: Env) => Promise<number> = env.copy(async (env: Env): Promise<number> => {
    return await incAndSend_Y(env.subst("A", "X", "B", "Y"));
  });
  let g: (env: Env, arg0: number) => Promise<void> = env.copy(async (env: Env, value: number) => {
    await incAndSend_X(env.subst("B", "X", "C", "Y"), value);
  });
  let c: (env: Env) => Promise<void> = env.copy(await compose_B(env.subst("A", "A", "B", "B", "C", "C"), env.copy(f), env.copy(g)));
  await c(env);
}
export async function Start_C(env: Env): Promise<number> {
  let g: (env: Env) => Promise<number> = env.copy(async (env: Env): Promise<number> => {
    return await incAndSend_Y(env.subst("B", "X", "C", "Y"));
  });
  let c: (env: Env) => Promise<number> = env.copy(await compose_C(env.subst("A", "A", "B", "B", "C", "C"), env.copy(g)));
  return await c(env);
}
